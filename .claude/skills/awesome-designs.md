...
## Calendar Monthly View Pattern

Implemented as a Filament page: `app/Filament/Pages/CalendarPage.php`
View: `resources/views/filament/pages/calendar-page.blade.php`

### Design rules
- Calendar card uses Level-2 glass: `bg-gray-900/60 backdrop-blur-xl border border-white/10`
- Top shimmer line on every major glass panel: `h-px bg-gradient-to-r from-transparent via-amber-500/50 to-transparent`
- Ambient orbs sit in a `fixed inset-0 -z-10 pointer-events-none` wrapper behind everything
- 7-column CSS grid (`grid grid-cols-7`) for day headers and day cells
- Day cells: `min-h-[110px]` to accommodate up to 3 pills + overflow badge

### Color coding (smart distribution)
Priority order within a day cell — most actionable first:

| Status | Pill classes | Dot |
|--------|-------------|-----|
| `failed` | `bg-rose-500/20 text-rose-300 border-rose-500/30` | `bg-rose-400` |
| `running` | `bg-blue-500/20 text-blue-300 border-blue-500/30` | `bg-blue-400 animate-pulse` |
| `completed` | `bg-emerald-500/20 text-emerald-300 border-emerald-500/30` | `bg-emerald-400` |
| `pending` | `bg-slate-500/15 text-slate-400 border-slate-500/20` | `bg-slate-500` |

### Cell ambient tint (heat-map layer)
- Any failed run on the day → `bg-rose-500/5` tint on the cell
- Any running, no failures → `bg-blue-500/5`
- All completed → `bg-emerald-500/5`

### Special day states
- **Today** — date number gets amber filled circle: `size-7 rounded-full bg-amber-500 text-gray-950 font-bold shadow-[0_0_12px_rgba(251,191,36,0.6)]`
- **Selected** — cell gets `bg-amber-500/8 ring-1 ring-inset ring-amber-500/40`
- **Other month** — `opacity-35`
- **Weekend** — subtle `bg-white/[0.01]` dimming

### Max 3 pills per cell
Show `$runs->take(3)`, then render `+N` overflow badge: `text-[10px] font-semibold bg-white/8 rounded-md px-1.5 py-0.5 border border-white/8`

### Detail panel (below calendar on day click)
- Slides in via Alpine `x-transition` + `$el.scrollIntoView`
- Glass card: `bg-gray-900/70 backdrop-blur-xl`
- Per-run row: status dot → vertical connector line → workflow name + badge + timestamps + error block
- Error block: `bg-rose-500/8 border border-rose-500/20 font-mono text-rose-400`
- Dismiss: clicking the same date again toggles `selectedDate = null`

### Month stat pills (header)
Show aggregate counts for the visible month inline in the header:
```blade
<span class="flex items-center gap-1 rounded-full px-2.5 py-1 bg-emerald-500/15 text-emerald-400 border border-emerald-500/25">
    <span class="size-1.5 rounded-full bg-emerald-400"></span>{{ $completedCount }}
</span>
```
